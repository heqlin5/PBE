...
from clustercontrast.trainers_regdb import ClusterContrastTrainer_pretrain_camera_interC,ClusterContrastTrainer_pretrain_camera_interM,ClusterContrastTrainer_pretrain_joint
...
def main_worker(args):
    ...
    logs_dir_root = osp.join(args.logs_dir+'/'+'regdb')
    trial = args.trial
...
################################ IR2RGB
    ...
    ###########structure smooth soft
        ...
        for i in range(args.part):
            ...
            cluster_features_rgb_part_i = generate_cluster_features(pseudo_labels_rgb, features_rgb_part_[:, :, i])
        ...
        train_loader_rgb = get_train_loader_color(args, dataset_rgb, args.height, args.width,
                                        128, args.workers, args.num_instances, 100,
        ...
        for i in range(args.part):
            ...
            cluster_features_ir_part_i = generate_cluster_features(pseudo_labels_ir, features_ir_part_[:, :, i])
        ...
        if epoch < (stage_cmass+5):
            ...
            features_rgb_dict, features_rgb_dict_part, _ = extract_features_part(model, cluster_loader_rgb,
                                                                                 print_freq=50, mode=1)
            features_rgb_ = torch.cat([features_rgb_dict[f].unsqueeze(0) for f, _, _ in sorted(dataset_rgb.train)], 0)
            features_rgb_part_ = torch.cat(
                [features_rgb_dict_part[f].unsqueeze(0) for f, _, _ in sorted(dataset_rgb.train)], 0)
            # compute the cross-agreement
            ...
            features_ir_dict, features_ir_dict_part, _ = extract_features_part(model, cluster_loader_ir, print_freq=50,
                                                                               mode=2)
            features_ir_ = torch.cat([features_ir_dict[f].unsqueeze(0) for f, _, _ in sorted(dataset_ir.train)], 0)
            features_ir_part_ = torch.cat(
                [features_ir_dict_part[f].unsqueeze(0) for f, _, _ in sorted(dataset_ir.train)], 0)
            ...
            for i in range(args.part):
                ...
                features_ir_1 = F.softmax(features_ir_part_i[:, :distribute_cm_map_rgb.size(0)], dim=1)
                features_ir_2 = F.softmax(features_ir_part_i[:, distribute_cm_map_rgb.size(0):], dim=1)
            ...
            features_all_part_ = torch.cat((features_rgb_part_, features_ir_part_), dim=0)
...
##############################
            ...
            data_path='/home/haorui/hql/cross_modality/RegDB/'
            ...
            ngall = len(gall_label)
            gall_feat_fc = extract_gall_feat(model,gall_loader,ngall, args.part)
            # fc feature
