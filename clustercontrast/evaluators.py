...
def extract_cnn_feature_part(model, inputs,mode,embedding_on=False):
    inputs = to_torch(inputs).cuda()
    # inputs1 = inputs
    # print(inputs)
    outputs, outputs_part = model.module.extract_all_features(inputs,inputs,modal=mode)
    outputs = outputs.data.cpu()
    outputs_part = outputs_part.data.cpu()
    return outputs, outputs_part
...
def extract_features_part(model, data_loader, print_freq=50,flip=True,mode=0):
    model.cuda()
    model.eval()
    batch_time = AverageMeter()
    data_time = AverageMeter()
    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, _, _) in enumerate(data_loader):
            ...
            outputs, outputs_part = extract_cnn_feature_part(model, imgs,mode)

            flip = fliplr(imgs)
            # print(flip)
            outputs_flip, outputs_flip_part = extract_cnn_feature_part(model, flip,mode)

            for fname, output,output_flip,pid in zip(fnames, outputs,outputs_flip, pids):
                features[fname] =  (output.detach() + output_flip.detach())/2.0
                labels[fname] = pid

            for fname, output_part,output_flip_part,pid in zip(fnames, outputs_part,outputs_flip_part, pids):
                features_part[fname] =  (output_part.detach() + output_flip_part.detach())/2.0
                # labels[fname] = pid
    ...
    return features, features_part, labels
