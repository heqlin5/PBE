...
class embed_net_ori(nn.Module):
    def __init__(self, num_parts=3, num_classes=1000, no_local= 'on', gm_pool = 'on', arch='resnet50'):
        ...
        self.classifier_ir = nn.Linear(pool_dim, 565, bias=False)
        self.classifier_rgb = nn.Linear(pool_dim, 1065, bias=False)
        ...
        for i in range(self.num_parts):
            ...
            # name = 'classifier_ir' + str(i)
            getattr(self, name).bias.requires_grad_(False)
            getattr(self, name).apply(weights_init_kaiming)

            name = 'classifier_rgb' + str(i)
            setattr(self, name, nn.Linear(2048, 1000, bias=False))
            name = 'classifier_ir' + str(i)
            setattr(self, name, nn.Linear(2048, 1000, bias=False))
    ...
    def forward(self, x1, x2, modal=0,label_1=None,label_2=None,cid_rgb=None,cid_ir=None,invers_bn=False):
        ...
        if self.training:
            return feat,feat[:single_size],feat[single_size:],fs_p[:single_size], fs_p[single_size:], label_1,label_2,cid_rgb,cid_ir  #x_pool#, self.classifier(feat) feat_rgb,feat_ir

        else:
            
            return self.l2norm(feat), fs_p


    def extract_all_features(self, x1, x2, modal=0, label_1=None, label_2=None, cid_rgb=None, cid_ir=None, invers_bn=False):
        ...
        return self.l2norm(feat), fs_p  # self.l2norm(x_pool)#,

        


def agw(pretrained=False,no_local= 'down', num_parts=3, **kwargs):
    """Constructs a ResNet-50 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = embed_net_ori(no_local= 'down', gm_pool = 'on', num_parts=num_parts)
    return model