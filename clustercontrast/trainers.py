...
class ClusterContrastTrainer_pretrain_camera_interC(object):
    ...
    def train(self, epoch, data_loader_ir,data_loader_rgb, optimizer,intra_id_labels_rgb=None, intra_id_features_rgb=None,intra_id_labels_ir=None, intra_id_features_ir=None,
        ...
        for i in range(train_iters):
            ...
            loss_gce_rgb = self.criterion_pglr(f_out_rgb, f_out_rgb_part, labels_rgb, ca_rgb)
            loss_gce_ir = self.criterion_pglr(f_out_ir, f_out_ir_part, labels_ir, ca_ir)
            ...
            if (i + 1) % print_freq == 0:
                print('Epoch: [{}][{}/{}]\t'
                      ...
                      .format(epoch, i + 1, len(data_loader_rgb),
                              batch_time.val, batch_time.avg,
                              data_time.val, data_time.avg,
                              losses.val, losses.avg,loss_ir,loss_rgb,loss_contrast_ir,loss_contrast_rgb,loss_camera_ir.item(),loss_camera_rgb.item()))
    ...
    def train(self, epoch, data_loader_ir,data_loader_rgb, optimizer,intra_id_labels_rgb=None, intra_id_features_rgb=None,intra_id_labels_ir=None, intra_id_features_ir=None,
        ...
        for i in range(train_iters):
            ...
            if self.num_part > 0:
                ...
                if epoch >= 0:
                    for part in range(self.num_part):
                        loss_pce_rgb += self.criterion_aals(f_out_rgb_part[:, :, part], labels_rgb, ca_rgb[:, part])
                        loss_pce_ir += self.criterion_aals(f_out_ir_part[:, :, part], labels_ir, ca_ir[:, part])
                        loss_contrast_rgb += self.memory_rgb_part[part](f_out_rgb_part[:, :, part], labels_rgb)
                        loss_contrast_ir += self.memory_ir_part[part](f_out_ir_part[:, :, part], labels_ir)
                      
               
                loss_pce_rgb /= self.num_part
                loss_pce_ir /= self.num_part
                loss_contrast_rgb /= self.num_part
                loss_contrast_ir /= self.num_part
            ...
            if (i + 1) % print_freq == 0:
                print('Epoch: [{}][{}/{}]\t'
                      ...
                      .format(epoch, i + 1, len(data_loader_rgb),
                              batch_time.val, batch_time.avg,
                              data_time.val, data_time.avg,
                              losses.val, losses.avg,loss_ir,loss_rgb,loss_contrast_ir,loss_contrast_rgb,loss_gce_ir,loss_gce_rgb,loss_pce_ir,loss_pce_rgb))
