...
from clustercontrast.evaluators import Evaluator, extract_features, extract_features_part
...
def create_model(args):
    model = models.create(args.arch, num_features=args.features, norm=True, dropout=args.dropout,
                          num_parts=args.part, num_classes=0, pooling_type=args.pooling_type)
    # use CUDA
    model.cuda()
...
def fliplr(img):
    ...
    return img_flip
def extract_gall_feat(model,gall_loader,ngall,num_part):
    pool_dim=2048
    net = model
    net.eval()
    ...
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(gall_loader):
            ...
            feat_fc, feat_fc_part = net( input,input, 1)
            flip_input = Variable(flip_input.cuda())
            feat_fc_1, feat_fc_1_part = net( flip_input,flip_input, 1)
...
def extract_query_feat(model,query_loader,nquery,num_part):
    pool_dim=2048
    ...
    with torch.no_grad():
        for batch_idx, (input, label ) in enumerate(query_loader):
            ...
            flip_input = fliplr(input)
            input = Variable(input.cuda())
            feat_fc, feat_fc_part = net( input, input,2)
            flip_input = Variable(flip_input.cuda())
            feat_fc_1, feat_fc_1_part = net( flip_input,flip_input, 2)
            feature_fc = (feat_fc.detach() + feat_fc_1.detach())/2
            fnorm_fc = torch.norm(feature_fc, p=2, dim=1, keepdim=True)
            feature_fc = feature_fc.div(fnorm_fc.expand_as(feature_fc))
...
def compute_cross_agreement(features_g, features_p,k1,k2):
    print("Compute cross-agreement...")
    N, D, P = features_p.size()
    score = torch.FloatTensor()
    end = time.time()
    ranked_list_g = compute_jaccard_distance(features_g, k1=k1, k2=k2, search_option=3)

    for i in range(P):
        features_p_i = features_p[:, :,i].contiguous()
        ranked_list_p_i = compute_jaccard_distance(features_p_i, k1=k1, k2=k2, search_option=3)
...
def main_worker(args):
    ...
    def generate_cluster_features(labels, features):
        ...
        return centers
    global start_epoch, best_mAP, best_cmc_1, best_cmc_5, best_cmc_10, best_cmc_20, best_mINP

    model = create_model(args)



    args.logs_dir = osp.join(args.logs_dir+'/'+'sysu')

    start_time = time.monotonic()
    # cudnn.benchmark = True

    sys.stdout = Logger(osp.join(args.logs_dir, 'log.txt'))#str(start_epoch)+no
    print("==========\nArgs:{}\n==========".format(args))
    ...
    trainer_intrac = ClusterContrastTrainer_pretrain_joint(model, num_part=args.part)  
    trainer_interc = ClusterContrastTrainer_pretrain_camera_interC(model, num_part=args.part)  
    trainer_interm = ClusterContrastTrainer_pretrain_camera_interM(model, num_part=args.part)  
    stage_cmass = 30
    for epoch in range(start_epoch,args.epochs):
        ...
        for rgb_cam_id in sorted(rgb_cams):
            ...
            for ir_cam_id in sorted(ir_cams):
                ...
                features_rgb_dict, features_rgb_dict_part, _  = extract_features_part(model, cluster_loader_rgb, print_freq=50,mode=1)
                features_rgb = torch.cat([features_rgb_dict[f].unsqueeze(0) for f, _, _ in sorted(cam_data_rgb)], 0)
                features_rgb_part = torch.cat(
                    [features_rgb_dict_part[f].unsqueeze(0) for f, _, _ in sorted(cam_data_rgb)], 0)
                #compute the cross-agreement
                score_rgb = compute_cross_agreement(features_rgb, features_rgb_part,k1=args.k1,k2=args.k2)
                ...
                for i in range(args.part):
                    cluster_features_rgb_part_i = generate_cluster_features(pseudo_labels_rgb, features_rgb_part[:, :, i])
                    memory_rgb_part_i = ClusterMemory(model.module.num_features, num_cluster_rgb, temp=args.temp,
                                               momentum=args.momentum, use_hard=args.use_hard).cuda()
                    memory_rgb_part_i.features = F.normalize(cluster_features_rgb_part_i, dim=1).cuda()
                    memory_rgb_part.append(memory_rgb_part_i)
                ...
                features_ir_dict, features_ir_dict_part, _  = extract_features_part(model, cluster_loader_ir, print_freq=50,mode=2)
                features_ir = torch.cat([features_ir_dict[f].unsqueeze(0) for f, _, _ in sorted(cam_data_ir)], 0)
                features_ir_part = torch.cat(
                    [features_ir_dict_part[f].unsqueeze(0) for f, _, _ in sorted(cam_data_ir)], 0)
                # compute the cross-agreement
                score_ir = compute_cross_agreement(features_ir, features_ir_part,k1=args.k1,k2=args.k2)
                ...
                memory_ir_part = []
                for i in range(args.part):
                    cluster_features_ir_part_i = generate_cluster_features(pseudo_labels_ir,
                                                                            features_ir_part[:, :, i])
                    memory_ir_part_i = ClusterMemory(model.module.num_features, num_cluster_ir, temp=args.temp,
                                                      momentum=args.momentum, use_hard=args.use_hard).cuda()
                    memory_ir_part_i.features = F.normalize(cluster_features_ir_part_i, dim=1).cuda()
                    memory_ir_part.append(memory_ir_part_i)
                ...
                train_loader_ir = get_train_loader_ir(args, dataset_ir, args.height, args.width,
                                            ...
                                            # 64, args.workers, args.num_instances, 50,
                ...
                train_loader_rgb = get_train_loader_color(args, dataset_rgb, args.height, args.width,
                                        ...
                                        # 32, args.workers, args.num_instances, 50,
                ...
                trainer_intrac.memory_ir_part = memory_ir_part
                trainer_intrac.memory_rgb = memory_rgb
                trainer_intrac.memory_rgb_part = memory_rgb_part
                train_loader_ir.new_epoch()
                time.sleep(1)
                train_loader_rgb.new_epoch()
                time.sleep(1)

                trainer_intrac.train(epoch, train_loader_ir,train_loader_rgb, optimizer, print_freq=10, train_iters=len(train_loader_rgb),score_rgb = score_rgb,score_ir=score_ir)
...
#############stage2
        ...
        features_rgb_part_ = torch.cat([features_rgb_dict_part[f].unsqueeze(0) for f, _, _ in sorted(dataset_rgb.train)], 0)

        # compute the cross-agreement
        score_rgb = compute_cross_agreement(features_rgb_, features_rgb_part_, k1=args.k1, k2=args.k2)
        ...
        # print('features_rgb_shape1',features_rgb.shape)
        ...
        features_rgb_part = []
        for i in range(args.part):
            features_rgb_part_i = F.normalize(features_rgb_part_[:, :, i]).cuda()
            features_rgb_part_i = getattr(model.module, 'classifier_rgb' + str(i))(features_rgb_part_i) * 20  
            features_rgb_part.append(features_rgb_part_i)
            features_rgb_1 = F.softmax(features_rgb_part_i[:, :rgb_softmax_dim[0]], dim=1)
            features_rgb_2 = F.softmax(
                features_rgb_part_i[:, rgb_softmax_dim[0]:rgb_softmax_dim[1] + rgb_softmax_dim[0]],
                dim=1)
            features_rgb_3 = F.softmax(features_rgb_part_i[:,
                                       rgb_softmax_dim[1] + rgb_softmax_dim[0]:rgb_softmax_dim[1] + rgb_softmax_dim[0] +
                                                                               rgb_softmax_dim[2]], dim=1)
            features_rgb_4 = F.softmax(
                features_rgb_part_i[:, rgb_softmax_dim[1] + rgb_softmax_dim[0] + rgb_softmax_dim[2]:],
                dim=1) 
            features_rgb_part_i = torch.cat((features_rgb_1, features_rgb_2, features_rgb_3, features_rgb_4), dim=1)
            features_rgb = torch.cat((features_rgb, features_rgb_part_i), dim=1)
        features_rgb_part = torch.stack(features_rgb_part, dim=-1)
        print("feature rgb shape for clustering", features_rgb.shape)
        ...
        features_ir_part_ = torch.cat(
            [features_ir_dict_part[f].unsqueeze(0) for f, _, _ in sorted(dataset_ir.train)], 0)
   
        # compute the cross-agreement
        score_ir = compute_cross_agreement(features_ir_, features_ir_part_, k1=args.k1, k2=args.k2)
        ...
        features_ir_part = []
        for i in range(args.part):
            features_ir_part_i = F.normalize(features_ir_part_[:,:,i]).cuda()
            features_ir_part_i = getattr(model.module, 'classifier_ir' + str(i))(features_ir_part_i)*20  
            features_ir_part.append(features_ir_part_i)
            features_ir_1 = F.softmax(features_ir_part_i[:, :ir_softmax_dim[0]], dim=1)
            features_ir_2 = F.softmax(features_ir_part_i[:, ir_softmax_dim[0]:], dim=1)
            features_ir_part_i = torch.cat((features_ir_1, features_ir_2), dim=1)
            features_ir = torch.cat((features_ir, features_ir_part_i), dim=1)
        features_ir_part = torch.stack(features_ir_part, dim=-1)
        print("feature ir shape for clustering", features_ir.shape)
...
################################ IR2RGB
        ...
        if epoch >= stage_cmass:
            lp_feat_rgb = features_rgb_.cpu().data
            lp_feat_ir =  features_ir_.cpu().data
    ...
    ###########structure smooth soft
        ...
        memory_rgb_part = []
        for i in range(args.part):
            cluster_features_rgb_part_i = generate_cluster_features(pseudo_labels_rgb,
                                                                   features_rgb_part_[:, :, i])
            memory_rgb_part_i = ClusterMemory(model.module.num_features, num_cluster_rgb, temp=args.temp,
                                             momentum=args.momentum, use_hard=args.use_hard).cuda()
            memory_rgb_part_i.features = F.normalize(cluster_features_rgb_part_i, dim=1).cuda()
            memory_rgb_part.append(memory_rgb_part_i)
        ...
        train_loader_rgb = get_train_loader_color(args, dataset_rgb, args.height, args.width,
                                        ...
                                        # 32, args.workers, args.num_instances, 200,
                                        trainset=pseudo_labeled_dataset_rgb, no_cam=args.no_cam,train_transformer=train_transformer_rgb,train_transformer1=train_transformer_rgb1)
        ...
        if epoch >= stage_cmass:
            ...
            trainer_interc.memory_ir_part = memory_ir_part
            trainer_interc.memory_rgb = memory_ir
            trainer_interc.memory_rgb_part = memory_ir_part
        else:
            trainer_interc.memory_ir = memory_ir
            trainer_interc.memory_ir_part = memory_ir_part
            trainer_interc.memory_rgb = memory_rgb
            trainer_interc.memory_rgb_part = memory_rgb_part
        ...
        trainer_interc.train(epoch, train_loader_ir,train_loader_rgb, optimizer,
            ...
            all_label_rgb=pseudo_labels_rgb,all_label_ir=pseudo_labels_ir,cams_ir=cams_ir,cams_rgb=cams_rgb,
                      print_freq=args.print_freq, train_iters=len(train_loader_ir),score_rgb=score_rgb,score_ir=score_ir)
        ...
        if epoch < (stage_cmass+10):
            ...
            features_rgb_dict, features_rgb_dict_part, _ = extract_features_part(model, cluster_loader_rgb, print_freq=50,mode=1)
            features_rgb_ = torch.cat([features_rgb_dict[f].unsqueeze(0) for f, _, _ in sorted(dataset_rgb.train)], 0)
            features_rgb_part_ = torch.cat(
                [features_rgb_dict_part[f].unsqueeze(0) for f, _, _ in sorted(dataset_rgb.train)], 0)
            # print(features_rgb_.shape)
            # compute the cross-agreement
            score_rgb = compute_cross_agreement(features_rgb_, features_rgb_part_, k1=args.k1, k2=args.k2)
            ...
            for i in range(args.part):
                features_rgb_part_i = F.normalize(features_rgb_part_[:, :, i]).cuda()
                features_rgb_part_i = getattr(model.module, 'classifier_rgb' + str(i))(
                    features_rgb_part_i) * 20  
                features_rgb_part.append(features_rgb_part_i)
                features_rgb_1 = F.softmax(features_rgb_part_i[:, :distribute_cm_map_rgb.size(0)].data, dim=1)
                features_rgb_2 = F.softmax(features_rgb_part_i[:, distribute_cm_map_rgb.size(0):].data, dim=1)
                features_rgb_part_i = torch.cat((features_rgb_1, features_rgb_2), dim=1)
                features_rgb = torch.cat((features_rgb, features_rgb_part_i), dim=1)
            features_rgb_part = torch.stack(features_rgb_part, dim=-1)
            print("feature rgb shape for clustering", features_rgb.shape)
            ...
            for i in range(args.part):
                features_ir_part_i = F.normalize(features_ir_part_[:, :, i]).cuda()
                features_ir_part_i = getattr(model.module, 'classifier_ir' + str(i))(
                    features_ir_part_i) * 20  
                features_ir_part.append(features_ir_part_i)
                print('stage3 ir_softmax_dim', ir_softmax_dim)
                print('stage3 distribute_cm_map_rgb', distribute_cm_map_rgb.size(0))
                features_ir_1 = F.softmax(features_ir_part_i[:, :ir_softmax_dim[0]], dim=1)
                features_ir_2 = F.softmax(features_ir_part_i[:, ir_softmax_dim[0]:], dim=1)
            ...
            features_all_part = torch.cat((features_rgb_part_,features_ir_part_),dim=0)
            ...
            memory_all_part = []
            for i in range(args.part):
                cluster_features_all_part_i = generate_cluster_features(pseudo_labels_all,
                                                                        features_all_part[:, :, i])

                memory_all_part_i = ClusterMemory(model.module.num_features, num_cluster_all, temp=args.temp,
                                                  momentum=args.momentum, use_hard=args.use_hard).cuda()
                memory_all_part_i.features = F.normalize(cluster_features_all_part_i, dim=1).cuda()
                memory_all_part.append(memory_all_part_i)


            trainer_interm.memory_ir = memory_all
            trainer_interm.memory_ir_part = memory_all_part
            trainer_interm.memory_rgb = memory_all
            trainer_interm.memory_rgb_part = memory_all_part
...
##############################
            ...
            data_path='/home/haorui/hql/cross_modality/SYSU_MM01/'
            query_img, query_label, query_cam = process_query_sysu(data_path, mode=mode)
            nquery = len(query_label)
            queryset = TestData(query_img, query_label, transform=transform_test, img_size=(args.img_w, args.img_h))
            query_loader = data.DataLoader(queryset, batch_size=args.test_batch, shuffle=False, num_workers=4)
            query_feat_fc = extract_query_feat(model,query_loader,nquery,args.part)
            for trial in range(1):
                ...
                gall_feat_fc = extract_gall_feat(model,trial_gall_loader,ngall,args.part)
...
#################################
            ...
            print('best_cmc_1',best_cmc_1)
            print('best_cmc_5',best_cmc_5)
            print('best_cmc_10',best_cmc_10)
            print('best_cmc_20',best_cmc_20)
            print('best_mINP',best_mINP)
############################
        lr_scheduler.step()
